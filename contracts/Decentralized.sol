...
modifier whenNotFrozen(address target) {
  ...
  modifier whenFrozen(address target){
    require(frozen[target],"tokens are not freezee");
    _;
  }
...
contract chargeV2 is Charge{
	using SafeMath for uint256;
	uint256 public basepercent1=1;
	uint256 public basepercent2=2;
	uint256 public basepercent3=3;
	...
	mapping (address => Deposit) public depositData;
	mapping (address => User) public users;
	mapping (address=>REF)public refusers;
	event NewDeposit(address indexed user, uint256 amount,uint256 duration);
	...
	function invest(uint256 _numberOfTokens,uint256 _value) public {
		...
		emit NewDeposit(msg.sender, _numberOfTokens,_value);
	}
	...
	function withdraw() public {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
		    if (user.deposits[i].withdrawn < user.deposits[i].amount.mul(user.deposits[i].basepercent).div(100).
		    mul(user.deposits[i].lockPeriod)) {
					dividends = (user.deposits[i].amount.mul(users[msg.sender].deposits[i].basepercent).div(100))
						.mul(block.timestamp.sub(user.deposits[i].start))
						.div(TIME_STEP);
	         	 user.deposits[i].start=now;
					if (user.deposits[i].withdrawn.add(dividends) > user.deposits[i].amount.mul(user.deposits[i].basepercent).div(100).
					mul(user.deposits[i].lockPeriod)) {
					dividends = (user.deposits[i].amount.mul(user.deposits[i].basepercent).div(100).mul(user.deposits[i].lockPeriod)).
	...
	for (uint256 i = 0; i < user.deposits.length; i++) {
		    if (user.deposits[i].withdrawn < user.deposits[i].amount.mul(user.deposits[i].basepercent).div(100).
		    mul(user.deposits[i].lockPeriod)) {
					dividends = (user.deposits[i].amount.mul(users[userAddress].deposits[i].basepercent).div(100))
						.mul(block.timestamp.sub(user.deposits[i].start))
						.div(TIME_STEP);
					if (user.deposits[i].withdrawn.add(dividends) > user.deposits[i].amount.mul(user.deposits[i].basepercent).div(100).
					mul(user.deposits[i].lockPeriod)) {
    ...
    function buyTokens(uint256 _numberOfTokens,address payable _refferer,uint256 _value) public payable returns(bool success){
        ...
        require(msg.value==(_numberOfTokens.div(1000000)).mul(p),"Not Enough TRX");
